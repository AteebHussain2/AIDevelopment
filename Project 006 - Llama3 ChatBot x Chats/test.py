...
for message in st.session_state.chat_history:
    if message["role"] == "user":
        message(message["content"], is_user=True, key=f"{message['role']}_{message['content']}")
    else:
        message(message["content"], key=f"{message['role']}_{message['content']}")

# Input field for user message
user_prompt = st.chat_input("Ask LLAMA 3.1")
if user_prompt:
    message(user_prompt, is_user=True, key=f"user_{user_prompt}")
    ...
    message(assistant_response, key=f"assistant_{assistant_response}")
