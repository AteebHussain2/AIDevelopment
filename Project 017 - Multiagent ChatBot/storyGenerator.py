...
write_task = Task(
    ...
    tools=[search_tool],
    output_file="data/temp-story.md",
)
...
def StoryGenerator(topic, retry_limit=3):
    ...
    try:
        ...
        story_details = research.kickoff(
            inputs={"topic": topic}
        ...
        story_content = write.kickoff(
            inputs={"topic": topic, "findings": story_details}
        )  # Writing Story
        ...
        validation_result = validation.kickoff(
            inputs={"topic": topic, "findings": story_details, "content": story_content}
        )
        ...
        if int(validation_result) == 1:
            ...
            temp_story_path = os.path.join(output_path, "temp-story.md")
            final_story_path = os.path.join(output_path, f"{title}_{timestamp}.md")

            if os.path.exists(temp_story_path):
                shutil.move(temp_story_path, final_story_path)
                print(f"Story has been successfully saved as '{final_story_path}'")
            else:
                print(f"Error: The file '{temp_story_path}' does not exist.")

            story_generator = {
                "topic": topic,
                "title": title,
                "validation_result": validation_result,
                "findings": story_details,
            ...
            temp_story_path = os.path.join(base_path, "data", "temp-story.md")
            cache_story_path = os.path.join(cache_path, f"{index}.md")

            if os.path.exists(temp_story_path):
                shutil.move(temp_story_path, cache_story_path)
                print(f"Logs saved at {cache_story_path}")
            else:
                print(f"Error: The file '{temp_story_path}' does not exist.")
