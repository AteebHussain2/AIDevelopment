...
write_task = Task(
    ...
    agent=writer_agent,
    async_execution=False,
    output_file="data/temp-recipe.md",
)
...
def RecipeGenerator(topic, retry_limit=3):
    ...
    try:
        ...
        recipe_details = research.kickoff(
            inputs={"topic": topic}
        ...
        content = write.kickoff(
            inputs={"topic": topic, "recipe_details": recipe_details}
        )  # Writing Recipe
        ...
        if validation_result == "1":
            title = title_generation.kickoff(inputs={"recipe_content": content})
            # Moving file to output path
            timestamp = generate_index()
            output_path = os.path.join(base_path, "data")
            temp_recipe_path = os.path.join(output_path, "temp-recipe.md")
            final_recipe_path = os.path.join(output_path, f"{title}_{timestamp}.md")

            if os.path.exists(temp_recipe_path):
                shutil.move(temp_recipe_path, final_recipe_path)
                print(f"Recipe has been successfully saved as '{final_recipe_path}'")

            else:
                print(f"Error: The file '{temp_recipe_path}' does not exist.")

            with open(final_recipe_path, "r") as f:
                content = f.read()
            ...
            temp_recipe_path = os.path.join(base_path, "data", "temp-recipe.md")
            cache_recipe_path = os.path.join(cache_path, f"{index}.md")

            if os.path.exists(temp_recipe_path):
                shutil.move(temp_recipe_path, cache_recipe_path)
                print(f"Logs saved at {cache_recipe_path}")
